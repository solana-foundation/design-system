...
import { cssVariablesTheme } from '@/lib/code-block-themes';
...
async function getHighlighter(): Promise<HighlighterCore> {
    if (highlighterInstance && currentThemeVersion === THEME_VERSION) return highlighterInstance;
...
async function ensureLanguage(highlighter: HighlighterCore, lang: string): Promise<boolean> {
    ...
    try {
        ...
        await highlighter.loadLanguage(
            (langModule.default ?? langModule) as Parameters<HighlighterCore['loadLanguage']>[0],
        );
 ...
 */
export function useShiki(code: string, language: string, decorations?: DecorationItem[]) {
