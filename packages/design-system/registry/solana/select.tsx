...
function collectItemRegistry(
  children: ReactNode
): Map<string, ItemRegistryEntry> {
...
const selectItemIconSizes: Record<SelectSize, React.CSSProperties> = {
  xl: {
...
const selectIndicatorSizes: Record<SelectSize, string> = {
...
export function Select({
  ...
  const selectedIcon =
    !multiple && typeof currentValue === "string"
      ? itemRegistry.get(currentValue)?.icon
      : null;
  ...
  const trigger = (
    ...
    >
      ...
      {selectedIcon && (
        <TriggerIconWrapper style={config.triggerIconStyle}>
          {selectedIcon}
        </TriggerIconWrapper>
      )}
      ...
      <BaseSelect.Value
        ...
        placeholder={
          <span className="text-[var(--input-placeholder-color)]">
            {placeholder}
          </span>
        }
      />
