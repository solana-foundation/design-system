...
function HeroStats() {
    return (
        <Section description="Proportional figures for static display — tighter, more natural." title="Hero Stats">
...
function ColorTokens() {
    return (
        <Section description="Semantic tokens aliasing status-success-text and status-error-text." title="Value Colors">
            <div className="grid grid-cols-3 gap-6">
                {[
                    ...
                    <div className="rounded-xl border border-border-medium p-6 text-center" key={t.label}>
        ...
        >
            <div className="grid grid-cols-2 gap-8">
                {[false, true].map(tabular => (
                    <div className="rounded-xl border border-border-medium p-6" key={String(tabular)}>
                ...
                <TableBody>
                    {tokens.map(t => (
                        <TableRow key={t.symbol}>
                            ...
                            <TableCell
                                align="right"
                                className={t.d1Up ? 'text-number-positive' : 'text-number-negative'}
                                numeric
                            >
                                {t.d1}
                            </TableCell>
                            <TableCell
                                align="right"
                                className={t.d7Up ? 'text-number-positive' : 'text-number-negative'}
                                numeric
                            >
                                {t.d7}
                            </TableCell>
...
function LiveCounter() {
    ...
    return (
        <Section description="Monospaced for animation, proportional for static." title="Live Counter">
            <div className="grid grid-cols-2 gap-8">
                <div className="rounded-xl border border-border-medium p-8">
                    <p className="mb-2 text-headline-md text-text-medium">Proportional (static)</p>
                    <p className="mb-1 text-body-sm text-text-low">TPS</p>
                    <p className="inline-block text-display text-text-extra-high">{formatted}</p>
                </div>
                <div className="rounded-xl border border-border-medium p-8">
                    <p className="mb-2 text-headline-md text-text-medium">Tabular (live)</p>
                    <p className="mb-1 text-body-sm text-text-low">TPS</p>
...
function NumberScale() {
    return (
        <Section description="Same number across the type scale with tabular figures." title="Number Scale">
            <div className="rounded-xl border border-border-medium p-6">
                {typeScale.map((t, i) => (
                    ...
                    >
                        ...
                        <span className={`${t.cls} text-text-extra-high tabular-nums`}>$1,234,567.89</span>
                    </div>
                ))}
            </div>
        </Section>
...
function Overview() {
    return (
        <div className="max-w-5xl space-y-14 p-8">
            <div>
                <h1 className="mb-2 text-display text-text-extra-high">Numeric Typography</h1>
                <p className="max-w-2xl text-body-lg text-text-medium">
                    CSS utilities for rendering numbers in financial and data-dense interfaces. Built on Inter
                    Variable's OpenType features.
                </p>
            </div>
            <HeroStats />
            <ColorTokens />
