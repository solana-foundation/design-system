...
const AddonWrapper = ({
    ...
    <span
        ...
        style={{
            paddingLeft: position === 'leading' ? outerPaddingX : kind === 'static' ? '0' : innerPaddingX,
            paddingRight: position === 'leading' ? (kind === 'static' ? '0' : innerPaddingX) : outerPaddingX,
...
const resolveTrailingSlot = ({
    ...
    return {
        addon: trailingAddon,
        content: trailingAddonKind === 'interactive' ? normalizeInteractiveAddonControl(trailingAddon) : trailingAddon,
...
function addonSidePadding(
    ...
    return kind === 'static' ? `calc(${config.contentGap} / 2)` : config.addonInnerPadding;
}
...
function getContentStyle(
    ...
    return {
        ...
        paddingRight: addonSidePadding(opts.hasTrailing, opts.trailingKind, config, opts.trailingIsAction),
    };
}
...
const warnTrailingActionConflict = ({
    ...
    if (process.env.NODE_ENV !== 'production' && trailingAction && trailingAddon && !hasWarnedRef.current) {
...
export function AddonSelect({
    ...
    const selectedLabel = options.find(o => o.value === currentValue)?.label ?? String(currentValue ?? '');
    ...
    const padLeft = position === 'leading' ? config.contentPaddingX : config.addonInnerPadding;
    const padRight = position === 'leading' ? config.addonInnerPadding : config.contentPaddingX;
    ...
    return (
        <span
            ...
            style={{
                ...
                borderRadius:
                    position === 'leading' ? `${innerRadius} 0 0 ${innerRadius}` : `0 ${innerRadius} ${innerRadius} 0`,
            }}
        >
            {/* Display label — pointer-events-none, clicks pass through to select */}
            <span className={cn('pointer-events-none select-none text-text-high', config.textClass)}>
                {selectedLabel}
            </span>
...
export const TextInput = forwardRef<HTMLInputElement, TextInputProps>(
    (
        ...
        const trailingOuterPaddingX = resolvedTrailingIsAction ? config.addonInnerPadding : config.contentPaddingX;
        ...
        const inputWrapper = (
            ...
            >
                ...
                <span className="flex min-w-0 flex-1 items-center self-stretch" style={contentStyle}>
                    {iconLeft && <IconWrapper style={config.iconStyle}>{iconLeft}</IconWrapper>}
                    ...
                    {iconRight && <IconWrapper style={config.iconStyle}>{iconRight}</IconWrapper>}

                    {action && (
                        <span className="relative z-10 inline-flex shrink-0 items-center justify-center">{action}</span>
                    )}
                </span>

                {resolvedTrailingAddonContent && (
                    <AddonWrapper
                        innerPaddingX={resolvedTrailingIsAction ? config.contentGap : config.addonInnerPadding}
                        innerRadius={
                            resolvedTrailingIsAction ? `calc(${config.radius} - var(--input-border-width))` : undefined
                        }
                        isAction={resolvedTrailingIsAction}
                        kind={resolvedTrailingAddonKind}
            ...
            <Field.Root className="flex flex-col gap-1.5" disabled={disabled} invalid={!!error}>
                {label && (
                    <div className="flex items-center gap-1">
                        <Field.Label className={cn('font-medium text-text-high', config.labelClass)}>
                            {label}
                        </Field.Label>
                        {hint && (
                ...
                <AnimatePresence initial={false} mode="wait">
                    {error ? (
                        ...
                        >
                            <Field.Error
                                className={cn('text-[var(--input-error-text)]', config.descriptionClass)}
                                match
                            >
                                {error}
                            </Field.Error>
