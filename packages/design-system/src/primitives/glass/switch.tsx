...
export function Switch({
    ...
    const canRenderGlass = !disabled && controlSize.width > 0 && controlSize.height > 0;
    const classes = [
        'gds-switch',
    ...
    const activeLensWidth = glassLensW * ACTIVE_LENS_SCALE;
    const activeLensHeight = glassLensH * ACTIVE_LENS_SCALE;
    const renderedLensWidth = isActive ? activeLensWidth : glassLensW;
    const renderedLensHeight = isActive ? activeLensHeight : glassLensH;
    ...
    const renderedLensX = lensGeometry.lensX - (renderedLensWidth - glassLensW) / 2;
    const renderedLensY = lensGeometry.lensY - (renderedLensHeight - glassLensH) / 2;
                    ...
                    <span className="gds-switch__lens">
                        ...
                        {canRenderGlass && (
                            <GlassRefraction
                                blur={glassBlur}
                                chroma={glassChroma}
                                contentClassName="gds-switch__lens-content"
                                depth={glassDepth}
                                dome={glassDome}
                                edge={glassEdge}
                                filterHeight={filterHeight}
                                filterId={filterId}
                                filterVariableName="--gds-switch-lens-filter"
