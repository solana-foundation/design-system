...
const meta: Meta<typeof Button> = {
    ...
    argTypes: {
        ...
        radius: {
            ...
            description: 'Corner style. Default uses size-specific radius, Round is fully rounded.',
            table: {
                category: 'Appearance',
        ...
        asChild: {
            control: 'boolean',
            description: 'Renders styles on child element (for links). Motion props unsupported.',
            table: {
                category: 'Advanced',
                type: { summary: 'boolean' },
                defaultValue: { summary: 'false' },
...
const AnimatedCopyIcon = ({ copied }: { copied: boolean }) => (
    <AnimatePresence initial={false} mode="popLayout">
        ...
        >
            {copied ? <CheckIcon height={16} width={16} /> : <ClipboardDocumentIcon height={16} width={16} />}
        </motion.span>
    </AnimatePresence>
);
...
const CopyButtonDemo = () => {
    ...
    return (
        <Button iconLeft={<AnimatedCopyIcon copied={copied} />} onClick={handleClick} size="md" variant="secondary">
            {copied ? 'Copied!' : 'Copy'}
        </Button>
    );
};
...
export const Playground: Story = {
    ...
    render: args => {
        ...
        return <Button {...args} iconLeft={args.iconLeft || (needsDefaultIcon ? <PlusIcon /> : undefined)} />;
    },
};
    ...
    render: () => (
        <div className="flex w-[720px] flex-col gap-10 p-8">
            ...
            <section className="flex flex-col gap-4">
                <h4 className="font-medium text-text-low text-xs uppercase tracking-wide">Variants</h4>
            ...
            {/* Sizes */}
            <section className="flex flex-col gap-4">
                <h4 className="font-medium text-text-low text-xs uppercase tracking-wide">Sizes</h4>
            ...
            {/* Shapes */}
            <section className="flex flex-col gap-4">
                <h4 className="font-medium text-text-low text-xs uppercase tracking-wide">Shapes</h4>
                <div className="flex items-center gap-4 rounded-xl border border-border-medium p-6">
                    <div className="flex flex-col items-center gap-2">
            ...
            {/* States */}
            <section className="flex flex-col gap-4">
                <h4 className="font-medium text-text-low text-xs uppercase tracking-wide">States</h4>
                <div className="flex items-center gap-4 rounded-xl border border-border-medium p-6">
                    <div className="flex flex-col items-center gap-2">
                        <Button size="md" variant="primary">
                            Default
            ...
            {/* Icons */}
            <section className="flex flex-col gap-4">
                <h4 className="font-medium text-text-low text-xs uppercase tracking-wide">Icons</h4>
                <div className="flex items-center gap-4 rounded-xl border border-border-medium p-6">
                    <div className="flex flex-col items-center gap-2">
            ...
            {/* Interactive Demo */}
            <section className="flex flex-col gap-4">
                <h4 className="font-medium text-text-low text-xs uppercase tracking-wide">Interactive Demo</h4>
                <div className="flex flex-col items-start gap-3 rounded-xl border border-border-medium p-6">
                    <CopyButtonDemo />
                    <p className="text-body-sm text-text-medium">Click to see blur+scale icon transition</p>
                </div>
            </section>
        </div>
    ),
