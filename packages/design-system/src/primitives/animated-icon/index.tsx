...
type MotionState = Exclude<ComponentProps<typeof motion.span>['exit'], undefined>;

const transitions: Record<AnimatedIconPreset, ComponentProps<typeof motion.span>['transition']> = {
 ...
 * />
 */
export const AnimatedIcon = ({ icon, iconKey, preset = 'default' }: AnimatedIconProps) => {
    const reduceMotion = useReducedMotion();
    const presetVariant = variantByPreset[preset];
    const staticVariant: MotionState =
        preset === 'default' ? { opacity: 1, scale: 1, filter: 'blur(0px)' } : { opacity: 1, scale: 1 };
