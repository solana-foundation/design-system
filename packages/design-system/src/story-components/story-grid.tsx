 ...
 */
export function StoryGrid({ children, columns = 5, className = '' }: StoryGridProps) {
    return <div className={`grid ${columnClasses[columns]} items-center gap-x-8 gap-y-6 ${className}`}>{children}</div>;
}
...
export function GridLabel({ children, header = false }: GridLabelProps) {
    if (header) {
        return <div className="text-center font-medium text-text-low text-xs uppercase tracking-wide">{children}</div>;
    }
