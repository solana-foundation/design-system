...
let highlighterPromise: Promise<import("@shikijs/core").HighlighterCore> | null =
  null;
...
async function tokenize(
  ...
  if (!loadedLangs.has(lang) && LANG_IMPORTS[lang]) {
    ...
    await hl.loadLanguage(
      (mod.default ?? mod) as Parameters<typeof hl.loadLanguage>[0],
    );
...
export function App() {
  ...
  const handleInsert = useCallback(async () => {
    ...
    code, language, syntax, shell, width, borderRadius, fontSize, lineHeight,
    showHeader, filename, showLineNumbers, addedLinesStr, removedLinesStr,
  ]);
