...
async function createCodeBlock(
  ...
  if (config.showLineNumbers) {
    ...
    const lineNumText = Array.from({ length: lineCount }, (_, i) =>
      String(i + 1),
    ).join("\n");
  ...
  if (hasDiff) {
    ...
    for (let i = 0; i < lineCount; i++) {
      ...
      if (char === "+") {
        diffMarkers.setRangeFills(offset, offset + 1, [
          solidPaint(colors.diffAddedMarker),
        ]);
      } else if (char === "-") {
        diffMarkers.setRangeFills(offset, offset + 1, [
          solidPaint(colors.diffRemovedMarker),
        ]);
      } else {
        diffMarkers.setRangeFills(offset, offset + 1, [
          solidPaint(colors.foreground, 0),
        ]);
      }
  ...
  for (const token of tokens) {
    ...
    try {
      codeText.setRangeFills(token.start, token.end, [
        solidPaint(token.color),
      ]);
  ...
  if (targetFrame) {
    ...
    figma.notify(`CodeBlock → "${targetFrame.name}" | ${applied} tokens colored, ${errors} errors (${fonts.regular.family})`);
  } else {
